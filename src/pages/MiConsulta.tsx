...
import { Role, Appointment, Patient, RiskAlert, SupervisionRequest } from '../types/clinical';
...
export const MiConsulta: React.FC<MiConsultaProps> = ({ userRole, appointments, patients, userName }) => {
  ...
  useEffect(() => {
    const refresh = () => setSupervisionRequests(supervisionRequestService.getRequests());
    window.addEventListener('brevemente_supervision_request_changed', refresh);
    return () => window.removeEventListener('brevemente_supervision_request_changed', refresh);
  }, []);
  const pendingRequests = supervisionRequests.filter(r => r.status === 'pendiente');
    ...
    <div className="space-y-6 animate-fadeIn text-xs text-slate-655 font-semibold leading-normal" data-tour="mi-consulta-main">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="bg-slate-50 p-4 rounded-xl border border-slate-200/60 space-y-3.5">
              <div className="flex justify-between items-start flex-wrap gap-2">
                ...
                <div className="text-right">
                  ...
                  <span className={`px-2.5 py-1 text-[9px] font-bold uppercase rounded-full border inline-block mt-1 ${sessionState === 'por_iniciar' ? 'bg-blue-50 text-blue-800 border-blue-200' :
        ...
        <div className="space-y-6">
          ...
          {/* PACIENTES QUE REQUIEREN ATENCIÓN URGENTE (alertas de riesgo activas) */}
          <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm space-y-4">
            ...
            <div className="space-y-3 font-semibold">
              {activeAlerts.length === 0 && pendingRequests.length === 0 ? (
                ...
                activeAlerts.map(a => (
                  <div key={a.id} className="bg-red-50/30 border-red-200 rounded-lg p-3 space-y-1">
                    <span className="font-extrabold text-red-750 block">{a.patientName}</span>
              ...
              {pendingRequests.map(r => (
                <div key={r.id} className="bg-amber-50/30 border-amber-200 rounded-lg p-3 space-y-1">
                  <span className="font-extrabold text-amber-750 block">{r.patientName}</span>
                  <span className="text-[10px] text-slate-500 block">Solicitud de supervisión · por {r.therapistName}</span>
                  <p className="text-slate-600 font-medium text-[11px] mt-0.5">{r.reason}</p>
                  <div className="flex gap-2 pt-1">
                    {['supervisor', 'admin_clinical', 'admin_platform'].includes(userRole) ? (
                      <button
                        onClick={() => supervisionRequestService.attendRequest(r.id, { id: 'user-current', name: userName, role: userRole })}
                        className="text-[9px] font-bold text-amber-700 hover:underline"
                      >
                        Atender Solicitud →
                      </button>
                    ) : (
                      <span className="text-[9px] font-bold text-amber-500">Solicitada · pendiente de revisión</span>
                    )}
                  </div>
                </div>
              ))}
